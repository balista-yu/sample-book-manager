...
import com.book.manager.sample_book_manager.infrastructure.database.mapper.OperatorDynamicSqlSupport.Operator
import com.book.manager.sample_book_manager.infrastructure.database.mapper.OperatorDynamicSqlSupport.Operator.id
import com.book.manager.sample_book_manager.infrastructure.database.mapper.OperatorDynamicSqlSupport.Operator.email
import com.book.manager.sample_book_manager.infrastructure.database.mapper.OperatorDynamicSqlSupport.Operator.name
import com.book.manager.sample_book_manager.infrastructure.database.mapper.OperatorDynamicSqlSupport.Operator.password
import com.book.manager.sample_book_manager.infrastructure.database.mapper.OperatorDynamicSqlSupport.Operator.roleType
...
fun OperatorMapper.deleteByPrimaryKey(id_: Int) = delete { where{ id isEqualTo id_} }
...
fun OperatorMapper.insertMultiple(records: Collection<OperatorRecord>) = insertMultiple(this::insertMultiple, records, Operator) {
    map(id).toProperty("id")
    map(email).toProperty("email")
    map(password).toProperty("password")
    map(name).toProperty("name")
...
fun OperatorMapper.selectByPrimaryKey(id_: Int) = selectOne { where{ id isEqualTo id_} }
